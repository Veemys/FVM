...
select case (mode)
	case(0)
		calcdiv_exact = 3.0 * x**2 + 3.0 * y**2
	case default
		calcdiv_exact = dot_product(v, grad) + p * 3.0 * (x**2 + y**2) ! V * gradP + P * divV
end select
...
calcrot_exact = 3.0 * (x**2 + y**2) ! dVy/dx - dVx/dy
...
calclaplacian_exact = 12.0 * (x**2 + y**2)
