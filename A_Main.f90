...
integer 								:: i, j, k, NI, NJ, dradCorrectIter, maxiter_correct, gradient_scheme, mode
...
real(8) 								:: pressure, velocity, calcgrad_exact, calcdiv_exact, calcrot_exact, calclaplacian_exact
...
read(io, *) mode			! scheme for divergency computation
close(io)
...
do j = 0, NJ
	do  i = 0, NI
		...
		divexact(i, j) = calcdiv_exact(cellcenter(i, j, 1), cellcenter(i, j, 2), p(i, j), v(i, j, :), gradexact(i, j, :), mode)
...
call calcdiv(NI, NJ, v, p, div, grad, cellvolume, cellcenter, iface_center, jface_center, iface_vector, jface_vector, mode)
